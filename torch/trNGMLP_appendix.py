...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_num", type = int, default = 1, help = "Model k per loss")
    ...
    parser.add_argument("--source_size", type = int, default = 20, help = "source dataset size (ratio)")

    args = parser.parse_args()

    physical_device = tf.config.list_physical_devices("GPU")[args.device_id]
    ...
    output_dir = f"saved_models/NGMLP_{args.source_size}"
    log_dir = f'logs/{data}/ng'
    ...
    #target_X.shape[0]*20
    np.random.seed(2)
    random_indices1 = np.random.choice(pd.read_csv("../data/M4_train.csv").iloc[:,(1):].index, size=target_X.shape[0]*source_size, replace=False)
...
mase_pred ,mase_pred2 = transfer_FC_MASE(model_num,mase_models,True, MASE(target_y,y_train.shape[1]),2000,8,10,learning_rate)
...
mae_pred, mae_pred2 = transfer_FC(
    ...
    pt=10,
    lr_=learning_rate,
    model_path='../pretrain/pretrain_NG/',
...
#name = 'NBEATs_I+FC'
pd.DataFrame(np.array(mse_pred).reshape(1,-1)).to_csv(f'result/val/trNG_{data}_mse_pred.csv')
pd.DataFrame(np.array(mae_pred).reshape(1,-1)).to_csv(f'result/val/trNG_{data}_mae_pred.csv')
pd.DataFrame(np.array(mape_pred).reshape(1,-1)).to_csv(f'result/val/trNG_{data}_mape_pred.csv')
pd.DataFrame(np.array(smape_pred).reshape(1,-1)).to_csv(f'result/val/trNG_{data}_smape_pred.csv')
pd.DataFrame(np.array(mase_pred).reshape(1,-1)).to_csv(f'result/val/trNG_{data}_mase_pred.csv')
...
pd.DataFrame(performance).to_csv(f"result/trNGNLP_{data}_weight.csv")


# In[57]:


pd.DataFrame(np.array(mse_pred2).reshape(1,-1)).to_csv(f'result/test/trNG_{data}_mse_pred.csv')
pd.DataFrame(np.array(mae_pred2).reshape(1,-1)).to_csv(f'result/test/trNG_{data}_mae_pred.csv')
pd.DataFrame(np.array(mape_pred2).reshape(1,-1)).to_csv(f'result/test/trNG_{data}_mape_pred.csv')
pd.DataFrame(np.array(smape_pred2).reshape(1,-1)).to_csv(f'result/test/trNG_{data}_smape_pred.csv')
pd.DataFrame(np.array(mase_pred2).reshape(1,-1)).to_csv(f'result/test/trNG_{data}_mase_pred.csv')
