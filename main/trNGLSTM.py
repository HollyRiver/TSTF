...
def transfer_FC_MASE(model_num, models, trainable, lossf, epochs_, batch_size_, pt, lr_):
    ...
    for i in range(1, model_num + 1):
        ...
        # LSTM 레이어 추가
        lstm_output = layers.LSTM(128,return_sequences=True)(pretrained_output_reshaped ) 
        lstm_output = layers.LSTM(64,return_sequences=True)(lstm_output)# LSTM 레이어 추가
        #lstm_output = layers.LSTM(32,return_sequences=True)(lstm_output)# LSTM 레이어 추가
...
# In[46]:
mae_models=train_bagging_models_G(model_num,'mae',2000,10,256,0.001)
mae_pred ,mae_pred2 = transfer_FC_MASE(model_num,mae_models,True, 'mae',2000,8,10,learning_late)

pd.DataFrame(np.array(mae_pred).reshape(1,-1)).to_csv(f'result/val/trNGLSTM_{data}_mae_pred.csv')
pd.DataFrame(np.array(mae_pred2).reshape(1,-1)).to_csv(f'result/test/trNGLSTM_{data}_mae_pred.csv')

del mae_models

mse_models=train_bagging_models_G(model_num,'mse',2000,10,256,0.001)
mse_pred ,mse_pred2 = transfer_FC_MASE(model_num,mse_models,True, 'mse',2000,8,10,learning_late)

pd.DataFrame(np.array(mse_pred).reshape(1,-1)).to_csv(f'result/val/trNGLSTM_{data}_mse_pred.csv')
pd.DataFrame(np.array(mse_pred2).reshape(1,-1)).to_csv(f'result/test/trNGLSTM_{data}_mse_pred.csv')

del mse_models

smape_models=train_bagging_models_G(model_num,SMAPE(),2000,10,256,0.001)
smape_pred ,smape_pred2 = transfer_FC_MASE(model_num,smape_models,True, SMAPE(),2000,8,10,learning_late)

pd.DataFrame(np.array(smape_pred).reshape(1,-1)).to_csv(f'result/val/trNGLSTM_{data}_smape_pred.csv')
pd.DataFrame(np.array(smape_pred2).reshape(1,-1)).to_csv(f'result/test/trNGLSTM_{data}_smape_pred.csv')

del smape_models

mase_models=train_bagging_models_G(model_num,MASE(y_train,y_train.shape[1]),2000,10,256,0.001)
mase_pred ,mase_pred2 = transfer_FC_MASE(model_num,mase_models,True, MASE(target_y,y_train.shape[1]),2000,8,10,learning_late)

pd.DataFrame(np.array(mase_pred).reshape(1,-1)).to_csv(f'result/val/trNGLSTM_{data}_mase_pred.csv')
pd.DataFrame(np.array(mase_pred2).reshape(1,-1)).to_csv(f'result/test/trNGLSTM_{data}_mase_pred.csv')

del mase_models

mape_models=train_bagging_models_G(model_num,'mape',2000,10,256,0.001)
mape_pred ,mape_pred2 = transfer_FC_MASE(model_num,mape_models,True, 'mape',2000,8,10,learning_late)

pd.DataFrame(np.array(mape_pred).reshape(1,-1)).to_csv(f'result/val/trNGLSTM_{data}_mape_pred.csv')
pd.DataFrame(np.array(mape_pred2).reshape(1,-1)).to_csv(f'result/test/trNGLSTM_{data}_mape_pred.csv')
