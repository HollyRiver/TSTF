...
LOG_DIR = os.path.join('logslstm', data)
...
#################################################################################
def transfer_LSTM(model_num, trainable, lossf, epochs_, batch_size_, pt, lr_,
    ...
    for i in range(1, model_num + 1):
        ...
        x = layers.LSTM(128,return_sequences=True)(pretrained_output_reshaped)
...
def transfer_LSTM_SMAPE(model_num, trainable, lossf, epochs_, batch_size_, pt, lr_,
...
def transfer_LSTM_MASE(model_num, trainable, lossf, epochs_, batch_size_, pt, lr_):
    history_mapes_G, model_pred_val, model_pred_test = [], [], []
    ...
    for i in range(1, model_num + 1):
        ...
        inputs = Input(shape=(target_X.shape[1], 1))
        flat_inp = layers.Reshape((target_X.shape[1],), name='transfer_flatten')(inputs)
        pretrained_output = pretrained_model(flat_inp)                         # (B, T_out * d_model)
        ...
        model_instance = Model(inputs=inputs, outputs=out)
        model_instance.compile(optimizer='adam', loss=lossf)
...
os.makedirs('resultlstm/val', exist_ok=True)
os.makedirs('resultlstm/test', exist_ok=True)

mase_pred, mase_pred2 = transfer_LSTM_MASE(model_num, True, MASE(target_y, y_train.shape[1]), 2000, 8, 10, learning_late)
pd.DataFrame(np.array(mase_pred).reshape(1, -1)).to_csv(f'resultlstm/val/trTFLSTM_{data}_mase_pred.csv')
pd.DataFrame(np.array(mase_pred2).reshape(1, -1)).to_csv(f'resultlstm/test/trTFLSTM_{data}_mase_pred.csv')

mae_pred, mae_pred2 = transfer_LSTM(
    model_num=model_num, trainable=True, lossf='mae',
    epochs_=2000, batch_size_=8, pt=10, lr_=learning_late
)
pd.DataFrame(np.array(mae_pred).reshape(1, -1)).to_csv(f'resultlstm/val/trTFLSTM_{data}_mae_pred.csv')
pd.DataFrame(np.array(mae_pred2).reshape(1, -1)).to_csv(f'resultlstm/test/trTFLSTM_{data}_mae_pred.csv')

mse_pred, mse_pred2 = transfer_LSTM(
    model_num=model_num, trainable=True, lossf='mse',
    epochs_=2000, batch_size_=8, pt=10, lr_=learning_late
)
pd.DataFrame(np.array(mse_pred).reshape(1, -1)).to_csv(f'resultlstm/val/trTFLSTM_{data}_mse_pred.csv')
pd.DataFrame(np.array(mse_pred2).reshape(1, -1)).to_csv(f'resultlstm/test/trTFLSTM_{data}_mse_pred.csv')

mape_pred, mape_pred2 = transfer_LSTM(
    model_num=model_num, trainable=True, lossf='mape',
    epochs_=2000, batch_size_=8, pt=10, lr_=learning_late
)
pd.DataFrame(np.array(mape_pred).reshape(1, -1)).to_csv(f'resultlstm/val/trTFLSTM_{data}_mape_pred.csv')
pd.DataFrame(np.array(mape_pred2).reshape(1, -1)).to_csv(f'resultlstm/test/trTFLSTM_{data}_mape_pred.csv')

smape_pred, smape_pred2 = transfer_LSTM_SMAPE(
    model_num=model_num, trainable=True, lossf=SMAPE(),
    epochs_=2000, batch_size_=8, pt=10, lr_=learning_late
)
pd.DataFrame(np.array(smape_pred).reshape(1, -1)).to_csv(f'resultlstm/val/trTFLSTM_{data}_smape_pred.csv')
pd.DataFrame(np.array(smape_pred2).reshape(1, -1)).to_csv(f'resultlstm/test/trTFLSTM_{data}_smape_pred.csv')
...
os.makedirs('resultlstm', exist_ok=True)
pd.DataFrame(performance).to_csv(f"resultlstm/trTFLSTM_{data}_weight.csv")
