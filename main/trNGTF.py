...
def transfer_FC_MASE(model_num, models, trainable, lossf, epochs_, batch_size_, pt, lr_):
 ...
 # 모델 컴파일
        early_stop = EarlyStopping(monitor='val_loss', patience=pt, verbose=0,
...
pd.DataFrame(np.array(mae_pred).reshape(1,-1)).to_csv(f'result/val/trNGTF_{data}_mae_pred.csv')
pd.DataFrame(np.array(mae_pred2).reshape(1,-1)).to_csv(f'result/test/trNGTF_{data}_mae_pred.csv')
...
pd.DataFrame(np.array(mse_pred).reshape(1,-1)).to_csv(f'result/val/trNGTF_{data}_mse_pred.csv')
pd.DataFrame(np.array(mse_pred2).reshape(1,-1)).to_csv(f'result/test/trNGTF_{data}_mse_pred.csv')
...
pd.DataFrame(np.array(smape_pred).reshape(1,-1)).to_csv(f'result/val/trNGTF_{data}_smape_pred.csv')
pd.DataFrame(np.array(smape_pred2).reshape(1,-1)).to_csv(f'result/test/trNGTF_{data}_smape_pred.csv')
...
pd.DataFrame(np.array(mase_pred).reshape(1,-1)).to_csv(f'result/val/trNGTF_{data}_mase_pred.csv')
pd.DataFrame(np.array(mase_pred2).reshape(1,-1)).to_csv(f'result/test/trNGTF_{data}_mase_pred.csv')
...
pd.DataFrame(np.array(mape_pred).reshape(1,-1)).to_csv(f'result/val/trNGTF_{data}_mape_pred.csv')
pd.DataFrame(np.array(mape_pred2).reshape(1,-1)).to_csv(f'result/test/trNGTF_{data}_mape_pred.csv')
