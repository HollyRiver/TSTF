...
# In[46]:
mae_models=train_bagging_models(model_num,'mae',2000,10,256,0.001)
mae_pred ,mae_pred2 = transfer_FC_MASE(model_num,mae_models,True, 'mae',2000,8,10,learning_late)

pd.DataFrame(np.array(mae_pred).reshape(1,-1)).to_csv(f'result/val/trNILSTM_{data}_mae_pred.csv')
pd.DataFrame(np.array(mae_pred2).reshape(1,-1)).to_csv(f'result/test/trNILSTM_{data}_mae_pred.csv')

del mae_models

mse_models=train_bagging_models(model_num,'mse',2000,10,256,0.001)
mse_pred ,mse_pred2 = transfer_FC_MASE(model_num,mse_models,True, 'mse',2000,8,10,learning_late)

pd.DataFrame(np.array(mse_pred).reshape(1,-1)).to_csv(f'result/val/trNILSTM_{data}_mse_pred.csv')
pd.DataFrame(np.array(mse_pred2).reshape(1,-1)).to_csv(f'result/test/trNILSTM_{data}_mse_pred.csv')

del mse_models

smape_models=train_bagging_models(model_num,SMAPE(),2000,10,256,0.001)
smape_pred ,smape_pred2 = transfer_FC_MASE(model_num,smape_models,True, SMAPE(),2000,8,10,learning_late)

pd.DataFrame(np.array(smape_pred).reshape(1,-1)).to_csv(f'result/val/trNILSTM_{data}_smape_pred.csv')
pd.DataFrame(np.array(smape_pred2).reshape(1,-1)).to_csv(f'result/test/trNILSTM_{data}_smape_pred.csv')
...
pd.DataFrame(np.array(mase_pred).reshape(1,-1)).to_csv(f'result/val/trNILSTM_{data}_mase_pred.csv')
pd.DataFrame(np.array(mase_pred2).reshape(1,-1)).to_csv(f'result/test/trNILSTM_{data}_mase_pred.csv')

del mase_models

mape_models=train_bagging_models(model_num,'mape',2000,10,256,0.001)
mape_pred ,mape_pred2 = transfer_FC_MASE(model_num,mape_models,True, 'mape',2000,8,10,learning_late)

pd.DataFrame(np.array(mape_pred).reshape(1,-1)).to_csv(f'result/val/trNILSTM_{data}_mape_pred.csv')
pd.DataFrame(np.array(mape_pred2).reshape(1,-1)).to_csv(f'result/test/trNILSTM_{data}_mape_pred.csv')
