...
LOG_DIR = os.path.join('logsmlp', data)
...
os.makedirs('resultmlp/val', exist_ok=True)
os.makedirs('resultmlp/test', exist_ok=True)

mase_pred, mase_pred2 = transfer_FC_MASE(model_num, True, MASE(target_y, y_train.shape[1]), 2000, 8, 10, learning_late)
pd.DataFrame(np.array(mase_pred).reshape(1, -1)).to_csv(f'resultmlp/val/trTFMLP_{data}_mase_pred.csv')
pd.DataFrame(np.array(mase_pred2).reshape(1, -1)).to_csv(f'resultmlp/test/trTFMLP_{data}_mase_pred.csv')
...
pd.DataFrame(np.array(mae_pred).reshape(1, -1)).to_csv(f'resultmlp/val/trTFMLP_{data}_mae_pred.csv')
pd.DataFrame(np.array(mae_pred2).reshape(1, -1)).to_csv(f'resultmlp/test/trTFMLP_{data}_mae_pred.csv')

mse_pred, mse_pred2 = transfer_FC(
    model_num=model_num, trainable=True, lossf='mse',
    epochs_=2000, batch_size_=8, pt=10, lr_=learning_late
)
pd.DataFrame(np.array(mse_pred).reshape(1, -1)).to_csv(f'resultmlp/val/trTFMLP_{data}_mse_pred.csv')
pd.DataFrame(np.array(mse_pred2).reshape(1, -1)).to_csv(f'resultmlp/test/trTFMLP_{data}_mse_pred.csv')

mape_pred, mape_pred2 = transfer_FC(
    model_num=model_num, trainable=True, lossf='mape',
    epochs_=2000, batch_size_=8, pt=10, lr_=learning_late
)
pd.DataFrame(np.array(mape_pred).reshape(1, -1)).to_csv(f'resultmlp/val/trTFMLP_{data}_mape_pred.csv')
pd.DataFrame(np.array(mape_pred2).reshape(1, -1)).to_csv(f'resultmlp/test/trTFMLP_{data}_mape_pred.csv')

smape_pred, smape_pred2 = transfer_FC_SMAPE(
    model_num=model_num, trainable=True, lossf=SMAPE(),
    epochs_=2000, batch_size_=8, pt=10, lr_=learning_late
)
pd.DataFrame(np.array(smape_pred).reshape(1, -1)).to_csv(f'resultmlp/val/trTFMLP_{data}_smape_pred.csv')
pd.DataFrame(np.array(smape_pred2).reshape(1, -1)).to_csv(f'resultmlp/test/trTFMLP_{data}_smape_pred.csv')
...
os.makedirs('resultmlp', exist_ok=True)
pd.DataFrame(performance).to_csv(f"resultmlp/trTFMLP_{data}_weight.csv")
